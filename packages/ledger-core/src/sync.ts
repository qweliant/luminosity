...
// initial bundle — sync isn't on the critical path for the first render.
...
// The singleton document. Which maps hang off it is the consuming app's
// business: each app declares its own typed containers against this doc
// (see each app's services/ binding file). Nothing in this package knows
// what an entry, a need or a part is, which is the whole reason it can be
// shared by two apps whose schemas have nothing in common.
...
/**
 * Signalling relay. Each app passes its own, because `import.meta.env` is
 * resolved per bundle — reading it from inside this package would silently
 * yield undefined in whichever app didn't happen to define it.
 */
let relayUrls: string[] = [];

export const configureSync = (opts: { relays: string[] }): void => {
  relayUrls = opts.relays.filter(Boolean);
};

const signalingUrls = (): string[] => relayUrls;

/** Shared spaces dial the same relays; they just land in different rooms. */
export const getRelays = (): string[] => relayUrls;
